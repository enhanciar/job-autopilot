...
until grep -q "closed" data/open_manual.log; do sleep 15; done      # user closed the sign-in window
for i in $(seq 1 900); do S=$(curl -s -m 10 "$API/runs?limit=30" | python3 -c "import sys,json;rs=[r for r in json.load(sys.stdin) if r['id']==118];print(rs[0]['status'] if rs else 'gone')"); [ "$S" != "running" ] && break; sleep 15; done; sleep 5
